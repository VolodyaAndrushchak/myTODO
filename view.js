var viewMainList = function(content, callback){
	...
	{
		htmlContent = htmlContent + "<li class='liTodo'><div class='TaskContent'  onclick='clickList(this);'> <div class='textContent'>"+ content[i].taskName + "</div><div class='time1'>" + content[i].time1 + "</div> <div class='time2'>" + content[i].time2 + "</div> <div class='priority'>" + content[i].priority + "</div><div class='delete' onclick= 'deleteTask(this);'> delete</div>  <div class='done'>done</div> </div> <div class = 'popUpGRUD'><div class = 'popInput'><input class='tastInput'>time <input class = 'time1Pop'>to<input class = 'time2Pop'>priority<select class = 'newPriority'><option>A</option><option>B</option><option>C</option></select></div><div class = 'editAdd' onclick= 'editADD(this)'>edit</div> </li> "
	}
	var statusDay = '';
	if(content.length <= 3)
	{
